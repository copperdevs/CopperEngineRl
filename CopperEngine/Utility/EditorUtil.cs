...
public static class EditorUtil
{
    ...
    public static bool DragMatrix4X4(string name, ref Matrix4x4 matrix, bool enabled = true)
    {
        ...
        if (ImGui.CollapsingHeader(name))
        {
            ...
            interacted =
                DragMatrix4X4Row($"Row One##{name}", ref matrix.M11, ref matrix.M12, ref matrix.M13, ref matrix.M14,
                    enabled) ||
                DragMatrix4X4Row($"Row Two##{name}", ref matrix.M21, ref matrix.M22, ref matrix.M23, ref matrix.M24,
                    enabled) ||
                DragMatrix4X4Row($"Row Three##{name}", ref matrix.M31, ref matrix.M32, ref matrix.M33, ref matrix.M34,
                    enabled) ||
                DragMatrix4X4Row($"Row Four##{name}", ref matrix.M41, ref matrix.M42, ref matrix.M43, ref matrix.M44,
                    enabled);
    ...
    private static bool DragMatrix4X4Row(string rowName, ref float itemOne, ref float itemTwo, ref float itemThree,
        ref float itemFour, bool enabled = true)
    {
        var interacted = false;
