...
public static class TextureUtil
{
    ...
    public static void Update<T>(Texture2D texture, ReadOnlySpan<T> pixels) where T : unmanaged =>
        Raylib.UpdateTexture(texture, pixels);

    /// <inheritdoc cref="Raylib.UpdateTexture"/>
    public static void Update<T>(Texture2D texture, T[] pixels) where T : unmanaged =>
        Raylib.UpdateTexture(texture, pixels);

    /// <inheritdoc cref="Raylib.UpdateTextureRec"/>
    public static void UpdateRec<T>(Texture2D texture, Rectangle rec, ReadOnlySpan<T> pixels) where T : unmanaged =>
        Raylib.UpdateTextureRec(texture, rec, pixels);

    /// <inheritdoc cref="Raylib.UpdateTextureRec"/>
    public static void UpdateRec<T>(Texture2D texture, Rectangle rec, T[] pixels) where T : unmanaged =>
        Raylib.UpdateTextureRec(texture, rec, pixels);
    ...
    public static void Draw(Texture2D texture, int posX, int posY, Color color) =>
        Raylib.DrawTexture(texture, posX, posY, color);
    ...
    public static void Draw(Texture2D texture, Vector2 pos, float rotation, float scale, Color color) =>
        Raylib.DrawTextureEx(texture, pos, rotation, scale, color);

    /// <inheritdoc cref="Raylib.DrawTextureRec"/>
    public static void DrawRec(Texture2D texture, Rectangle source, Vector2 pos, Color color) =>
        Raylib.DrawTextureRec(texture, source, pos, color);

    /// <inheritdoc cref="Raylib.DrawTexturePro"/>
    public static void DrawPro(Texture2D texture, Rectangle source, Rectangle dest, Vector2 origin, float rotation,
        Color color) => Raylib.DrawTexturePro(texture, source, dest, origin, rotation, color);

    /// <inheritdoc cref="Raylib.DrawTextureNPatch"/>
    public static void DrawNPatch(Texture2D texture, NPatchInfo nPatchInfo, Rectangle dest, Vector2 origin,
        float rotation, Color color) => Raylib.DrawTextureNPatch(texture, nPatchInfo, dest, origin, rotation, color);
